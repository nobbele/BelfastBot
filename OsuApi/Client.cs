...
namespace OsuApi
{
    public class Client
    {
        ...
        public static async Task<UserResult> GetUser(string user, int mode)
        {
            using (HttpClient httpClient = new HttpClient())
            {
                string json = await httpClient.GetStringAsync($"{BaseUrl}get_user?u={user}&k={"Give the token from config"}&m ={mode}");
